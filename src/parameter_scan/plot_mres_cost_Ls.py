...
parser = argparse.ArgumentParser(
    description=(
        ...
        "2) Time scan: m_res vs time (ONLY Shamir points + Möbius points used in the Möbius line)\n"
    )
)
...
parser.add_argument("--costs", required=True, help="Output PDF for m_res vs time (2 betas).")
...
for fp in args.hmc:
    p = parse_params(fp)
    hmc_lookup[params_key(p)] = load_hmc_cost(fp)
...
for fp in args.mres:
    ...
    bcs, bcs_err, ttraj, ttraj_err = hmc_lookup[k]
...
# ------------------------------------------------------------
# Panel 1: Ls scan (same logic/style as your shared plot_Ls)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Panel 2: Time scan (ONLY Shamir + Möbius points used in dotted line)
# ------------------------------------------------------------
...
def plot_time_panel(ax, entries_panel, title):
    ...
    # connect lines (same style as your merged script)
    connect(ax, sh_x, sh_y, "--", "black")
    connect(ax, mo_x, mo_y, ":", "C1")
...
def make_time_scan_figure(outname: str):
    ...
    for ax, beta in zip(axs, beta_vals):
        ...
        # match your merged script defaults
        ax.set_xlim(50, 1500)
