...
def make_models_VV(T_data, T_period, t0, t1):
    return [
        cf.Corr2(
            datatag="VV",
...
def fit_with_bootstrap_VV(
    ...
    models = make_models_VV(T_data, Nt_full, t0, t1)
    ...
    if boot_idx is None:
        ...
        for bs_fit in fitter.bootstrapped_fit_iter(datalist=bs_datalist):
            p = bs_fit.pmean
            p_gv = bs_fit.p
            m_v_b = float(np.asarray(p["m_v"])[0])
            AfitV_b = float(np.asarray(p["AfitV"])[0])
            m_v_err_b = float(gv.sdev(p_gv["m_v"][0]))
            AfitV_err_b = float(gv.sdev(p_gv["AfitV"][0]))
            chi2_b = float(bs_fit.chi2)

            bs.append("m_v", [m_v_b])
            bs.append("AfitV", [AfitV_b])
            bs_chi2.append(chi2_b)
            bootstrap_samples.append({
                "m_v": m_v_b,
                "m_v_err": m_v_err_b,
                "AfitV": AfitV_b,
                "AfitV_err": AfitV_err_b,
                "chi2": chi2_b,
            })
            n_success += 1
    else:
        ...
        for iboot, idx in enumerate(boot_idx):
            data_b = ds.avg_data(_dataset_from_samples({"VV": v_samples[idx]}))
            try:
