...
# Plotting
# ============================================================
def companion_plot_path(output_plot, suffix):
    path = Path(output_plot)
    return str(path.with_name(f"{path.stem}_{suffix}{path.suffix}"))


def configure_presentation_axes(ax):
    ax.set_xlabel(r"$(m_{\rm PS} w_0)^2$", fontsize=AXIS_LABEL_SIZE)
    ax.set_ylabel(r"$(f_{\rm PS} w_0)^2$", fontsize=AXIS_LABEL_SIZE)
    ...
    ax.xaxis.get_offset_text().set_size(TICK_LABEL_SIZE)
    ax.yaxis.get_offset_text().set_size(TICK_LABEL_SIZE)


def save_current_figure(output_plot):
...
def compute_plot_x_range(dw_points, wilson_points):
    all_x = np.array(
        [p["x"] for p in dw_points] + [p["x"] for p in wilson_points],
        dtype=float,
    )
    x_max = 1.05 * np.max(all_x)
    return x_max
...
def plot_points_and_fits(
    ...
    plot_fit_keys,
    output_plot,
    include_dw_points=True,
    include_wilson_points=True,
):
    ...
    x_limit_max = compute_plot_x_range(dw_points, wilson_points)

    fig, ax = plt.subplots(figsize=PRESENTATION_FIGSIZE, layout="constrained")

    if include_dw_points:
        dw_label_used = False
        for p in dw_points:
            label = "DWF data" if not dw_label_used else None
            dw_label_used = True
    ...
    x_sources = []
    if include_dw_points:
        x_sources.extend(p["x"] for p in dw_points)
    if include_wilson_points:
        x_sources.extend(p["x"] for p in wilson_points)
    if not x_sources:
        raise ValueError("No points selected for plotting.")

    x_grid = np.linspace(0.0, x_limit_max, 500)
    wilson_fit_label_used = False
    dw_fit_label_used = False
    ...
    for fit_key in plot_fit_keys:
        ...
        if fit_key.startswith("wilson_"):
            label = (
                "Wilson chiral continuum extrapolation"
                if not wilson_fit_label_used
                else None
            )
            wilson_fit_label_used = True
        elif fit_key in {"dw", "dw2"}:
            label = (
                "DWF chiral continuum extrapolation"
                if not dw_fit_label_used
                else None
            )
            dw_fit_label_used = True
        else:
            label = fit["label"]
    ...
    configure_presentation_axes(ax)
    ax.set_xlim(0.0, x_limit_max)
    ax.legend(loc="best", fontsize=LEGEND_FONT_SIZE, framealpha=0.92)
    save_current_figure(output_plot)
...
def main():
    ...
    wilson_only_plot = companion_plot_path(args.output_plot, "wilson_only")
    wilson_fit_keys = [key for key in PLOT_FITS if key.startswith("wilson_")]
    ...
    if wilson_fit_keys:
        plot_points_and_fits(
            ...
            all_fits=all_fits,
            plot_fit_keys=wilson_fit_keys,
            output_plot=wilson_only_plot,
            include_dw_points=False,
            include_wilson_points=True,
        )
    ...
    if wilson_fit_keys:
        print(f"✓ Saved Wilson-only plot → {wilson_only_plot}")
    print(f"✓ Saved combined plot → {args.output_plot}")
