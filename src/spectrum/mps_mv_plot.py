...
# MANUAL EXCLUSIONS
# ------------------------------------------------------------
# Remove points explicitly by (beta, m0).
#
# Example:
#   MANUAL_EXCLUDE_COMBINATIONS = [
#       (7.62, 0.012),
#       (7.70, 0.020),
#   ]
#
# These exclusions are applied to BOTH DWF/MDWF and Wilson points.
# If you want separate exclusion lists for each discretization, that
...
#(7.05, -0.85),
#(7.05, -0.857),
(7.05, -0.863), #alway remove
(7.05, -0.867), #alway remove
#(7.2, -0.78),
#(7.2, -0.79),
#(7.2, -0.794),
#(7.2, -0.799),
#(7.2, -0.803),
#(7.2, -0.808),
(7.2, -0.813), #alway remove
#(7.4, -0.74),
#(7.4, -0.75),
(7.4, -0.755), #alway remove
#(7.5, -0.71),
#(7.5, -0.72),
]

#these are for latest data
'''
(6.9, -0.87),
(6.9, -0.89),
(6.9, -0.9),
...
# ============================================================
def normalize_exclusion_pairs(exclusion_pairs, ndigits=12):
    """
    Convert (beta, m0) pairs into a normalized set so float comparisons are stable.
    """
    return {
        (round(float(beta), ndigits), round(float(m0), ndigits))
        for beta, m0 in exclusion_pairs
    }


def point_key_beta_m0(point, ndigits=12):
    return (
        round(float(point["beta"]), ndigits),
        round(float(point["m0"]), ndigits),
    )


def filter_points_by_beta_mass(points, exclusion_pairs):
    """
    Split points into kept and removed according to manual (beta, m0) exclusions.
    """
    exclusion_set = normalize_exclusion_pairs(exclusion_pairs)

    kept = []
    removed = []

    for p in points:
        if point_key_beta_m0(p) in exclusion_set:
            removed.append(p)
        else:
            kept.append(p)
...
def print_manually_removed_points(removed_points, label):
    if not removed_points:
        return

    print(f"Manually excluded {label} points:")
    for p in removed_points:
        extra = f", Ensemble={p['Ensemble']}" if "Ensemble" in p else ""
        print(
            f"  beta={p['beta']}, m0={p['m0']}{extra}, "
            f"x={(p['x'] if 'x' in p else float('nan')):.8g}"
        )
...
def solve_weighted_least_squares(design_matrix, y, ye, label):
    """
    Weighted least squares using only y-errors.
    """
    if np.any(ye <= 0):
...
def save_fit_results_json(
    ...
    payload = {
        ...
        "manual_exclude_combinations": [
            {"beta": float(beta), "m0": float(m0)}
            for beta, m0 in manual_exclusions
        ],
        "n_dw_points_used": len(dw_points_used),
        "n_wilson_points_used": len(wilson_points_used),
        "manually_excluded": {
            "dw": to_serializable(dw_points_removed),
            "wilson": to_serializable(wilson_points_removed),
        },
        "fits": {
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "continuum ansaetze. Points can be manually excluded by (beta, m0) using "
            "the MANUAL_EXCLUDE_COMBINATIONS list in the script."
        )
    )
    ...
    dw_points, dw_removed = filter_points_by_beta_mass(
        dw_points, MANUAL_EXCLUDE_COMBINATIONS
    )
    wilson_points, wilson_removed = filter_points_by_beta_mass(
        wilson_points, MANUAL_EXCLUDE_COMBINATIONS
    )

    print_manually_removed_points(dw_removed, "DWF/MDWF")
    print_manually_removed_points(wilson_removed, "Wilson")
    ...
    save_fit_results_json(
        ...
        plot_fit_keys=PLOT_FITS,
        dw_points_used=dw_points,
        wilson_points_used=wilson_points,
        dw_points_removed=dw_removed,
        wilson_points_removed=wilson_removed,
        manual_exclusions=MANUAL_EXCLUDE_COMBINATIONS,
        all_fits=all_fits,
    )
