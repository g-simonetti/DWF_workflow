...
def read_mres_file(path):
    """
    Read the residual mass fit file and return:
      mres, mres_err, chi2, reduced_chi2, plateau_start, plateau_end

    Expected data format (non-comment line):
      mres_fit  mres_fit_err  chi2  reduced_chi2  plateau_start  plateau_end
    """
    ...
    with open(path) as f:
        for line in f:
            ...
            vals = line.split()
            if len(vals) < 6:
                raise ValueError(f"Unexpected format in {path}: expected >= 6 columns, got {len(vals)}")

            mres, mres_err, chi2, chi2_red, pstart, pend = vals[:6]
            return float(mres), float(mres_err), float(chi2), float(chi2_red), int(pstart), int(pend)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate LaTeX longtable of ensemble parameters with residual masses (includes χ² and reduced χ²)."
    )
    ...
    for _, row in df.iterrows():
        try:
            ...
            mres, mres_err, chi2, chi2_red, pstart, pend = read_mres_file(path)
        except Exception as e:
            print(f"Warning: could not read m_res file for {row['name']}: {e}")
            mres = mres_err = chi2 = chi2_red = pstart = pend = np.nan
    ...
    header_line = (
        ...
        "$\\tilde{t}^{am_{\\rm res}}_{\\rm end}$ & $\\chi^2$ & $\\chi^2_\\text{red}$ \\\\\n"
    )

    with open(args.output_file, "w") as f:
        f.write("\\color{red}\n")
        # NOTE: added one column for chi^2 -> now 15 columns total
        f.write("\\begin{longtable}{|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|}\n")
        ...
        f.write(
            ...
            "$\\tilde{t}^{am_{\\rm res}}_{\\rm end}$, together with $\\chi^2$ and the reduced chi-square of the fit.}\n"
        )
        ...
        for _, r in df_out.iterrows():
            chi2_str = f"{r['mres_chi2']:.3f}" if np.isfinite(r["mres_chi2"]) else "--"
            chi2_red_str = f"{r['mres_chi2_red']:.3f}" if np.isfinite(r["mres_chi2_red"]) else "--"
            ...
            f.write(
                ...
                f"{r['am_res']} & {pstart_str} & {pend_str} & {chi2_str} & {chi2_red_str} \\\\\n"
            )
