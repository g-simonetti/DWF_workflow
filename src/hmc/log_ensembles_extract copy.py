...
Output columns:
  ...
  bcs bcs_err t_traj t_traj_err tau_int_plaq tau_int_plaq_err
...
def slice_therm_delta(x, therm, delta, n_keep):
    """Apply thermalization cut, subsample by delta, and cap to n_keep entries."""
    if x.size == 0:
        return x
    start = min(therm, x.size)
    idx = np.arange(start, min(start + n_keep, x.size), delta, dtype=int)
    return x[idx]
...
def ms_autocorr_time(x, c=5.0):
    ...
    ----------
    x : array_like
        Ordered time series (after thermalization).
    c : float, optional
        Window factor for the self-consistent window (default: 5).
    ...
    tau_err : float
        Madras–Sokal analytic error estimate.
    """
    ...
    # truncate at first negative (initial positive sequence)
    ...
    # Madras–Sokal variance of tau_int
...
def main():
    ...
    if plaq_pairs.size > 0:
        ...
        i_min, i_max = traj_idx.min(), traj_idx.max()
        n_total = i_max - i_min + 1
        full_series = np.full(n_total, np.nan)
        full_series[traj_idx - i_min] = plaq_vals
        for i in range(1, n_total):
            if np.isnan(full_series[i]):
                full_series[i] = full_series[i - 1]
        start = max(args.therm - i_min, 0)
        plaq_post_therm_vals = full_series[start:]
    else:
        plaq_post_therm_vals = np.array([], dtype=float)

    lengths = [
        data["fullbcs_raw"].size,
        data["fullbcs_incr"].size,
        data["traj_times"].size,
    ]
    n_max = int(max(lengths)) if lengths else 0
    usable = max(0, n_max - args.therm)
    n_traj = (usable // args.delta_traj) * args.delta_traj
    n_conf = n_traj // args.delta_traj

    # Apply cuts for bootstraps
    fullbcs_incr_s = slice_therm_delta(data["fullbcs_incr"], args.therm, args.delta_traj, n_traj)
    traj_times_s   = slice_therm_delta(data["traj_times"],   args.therm, args.delta_traj, n_traj)
    ...
    with open(args.output_file, "w") as f:
        f.write("name fullbcs fullbcs_err therm delta_traj n_conf "
                "bcs bcs_err t_traj t_traj_err tau_int_plaq tau_int_plaq_err "
                "length_traj n_steps accept_ratio\n")
        f.write(f"{args.name} {fullbcs_mean:.6g} {fullbcs_err:.6g} {args.therm} {args.delta_traj} {n_conf} "
                f"{bcs_mean:.6g} {bcs_err:.6g} {ttraj_mean:.6g} {ttraj_err:.6g} "
                f"{tau_est:.6g} {tau_err:.6g} {length_traj:.6g} {n_steps} {accept_ratio:.6g}\n")
    ...
    print(f"  n_max={n_max}, therm={args.therm}, delta={args.delta_traj}, n_traj={n_traj}, n_conf={n_conf}")
    print(f"  fullbcs(incr)={fullbcs_mean:.6g}±{fullbcs_err:.6g}, "
